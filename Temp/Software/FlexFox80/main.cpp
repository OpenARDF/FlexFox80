...
typedef enum
{
	...
	FINISH_TIME_VALIDATION,
	CURRENT_TIME_VALIDATION
} TimeValidation_t;
...
volatile bool g_use_rtc_to_time_event = true;
...
time_t validateTimeString(char* str, int8_t offsetHours, TimeValidation_t evalType);
    ...
    if(TCB3.INTFLAGS & TCB_CAPT_bm)
    {
		if(cnt++ == 299) /* check for 1-second interval */
		{
			...
			else
			{
				...
				{
					if(g_use_rtc_to_time_event && g_event_finish_epoch && !g_check_for_next_event && !g_shutting_down_wifi)
					{
						time(&temp_time);
				...
				if(g_event_enabled)
				{
					...
					{
						if(g_use_rtc_to_time_event)
						{
							if(g_event_start_epoch > 0) /* off the air - waiting for the start time to arrive */
							{
								time(&temp_time);
...
extern uint16_t g_fine_freq_adj;
			...
			case SB_MESSAGE_SET_STATION_ID:
			{
				if(sb_buff->fields[SB_FIELD1][0])
				{
					if(sb_buff->fields[SB_FIELD1][0] == '*')
					{
						...
						g_ee_mgr.updateEEPROMVar(StationID_text, (void*)"\0");
					}
					else
					{
						strcpy(g_tempStr, " ");
			...
			case SB_MESSAGE_CLOCK:
			{
				...
				if(!sb_buff->fields[SB_FIELD1][0] || sb_buff->fields[SB_FIELD1][0] == 'T')   /* Current time format "YYMMDDhhmmss" */
				{
					if(sb_buff->fields[SB_FIELD2][0])
					{
						...
						g_tempStr[12] = '\0';               /* truncate to no more than 12 characters */
 						...
 						time_t t = validateTimeString(g_tempStr, -g_utc_offset, CURRENT_TIME_VALIDATION);;
 						...
 						if(t)
 						{
							...
							else
							{
								...
								set_system_time(ds3231_get_epoch(NULL));
 								sprintf(g_tempStr, "Time: %s\n", convertEpochToTimeString(time(NULL), t, 50));
							...
							sb_send_string(g_tempStr);
 							...
 							else
 							{
								time_t current_time = time(NULL);
 								reportTimeTill(current_time, g_event_start_epoch, "Starts in: ", "In progress\n");
 								reportTimeTill(g_event_start_epoch, g_event_finish_epoch, "Lasts: ", NULL);
 								if(g_event_start_epoch < current_time)
 								{
 									reportTimeTill(current_time, g_event_finish_epoch, "Time Remaining: ", NULL);
 								}
								doprint = true;
 							}
						}
				...
				else if(sb_buff->fields[SB_FIELD1][0] == 'S')  /* Event start time */
				{
					strcpy(g_tempStr, sb_buff->fields[SB_FIELD2]);
 					time_t s = validateTimeString(g_tempStr, -g_utc_offset, START_TIME_VALIDATION);
				...
				{
 					strcpy(g_tempStr, sb_buff->fields[SB_FIELD2]);
 					time_t f = validateTimeString(g_tempStr, -g_utc_offset, FINISH_TIME_VALIDATION);
...
EC activateEventUsingCurrentSettings(SC* statusCode)
{
	...
	time_t now = time(NULL); // time(NULL);
...
void startEventUsingRTC(void)
{
	time_t current_epoch = ds3231_get_epoch(NULL);
	set_system_time(current_epoch);
	...
	{
		setupForFox(NULL, START_EVENT_WITH_STARTFINISH_TIMES);
		reportTimeTill(current_epoch, g_event_start_epoch, "Starts in: ", "In progress\n");

		if(g_event_start_epoch < current_epoch)
		{
			reportTimeTill(current_epoch, g_event_finish_epoch, "Time Remaining: ", NULL);
		}
		else
		{
			reportTimeTill(g_event_start_epoch, g_event_finish_epoch, "Lasts: ", NULL);
...
// 			g_id_interval_seconds = 60;
	...
	{
		g_on_the_air = g_intra_cycle_delay_time ? -g_intra_cycle_delay_time : g_on_air_seconds;
		g_use_rtc_to_time_event = false;
		g_event_commenced = true;                   /* get things running immediately */
		g_event_enabled = true;                     /* get things running immediately */
		g_last_status_code = STATUS_CODE_EVENT_STARTED_WAITING_FOR_TIME_SLOT;
	...
	{
		SC sc;
 		EC ec = launchEvent(&sc);
		 
		if(ec == ERROR_CODE_NO_ERROR)
		{
			g_use_rtc_to_time_event = true;
		}
	}
...
time_t validateTimeString(char* str, int8_t offsetHours, TimeValidation_t evalType)
{
	...
	if(evalType == START_TIME_VALIDATION)
	{
		minimumEpoch = MAX(current_epoch, MINIMUM_EPOCH);
	}
	else if(evalType == FINISH_TIME_VALIDATION)
	{
		minimumEpoch = MAX(g_event_start_epoch, current_epoch);
	}
	...
	if((len == 12) && (only_digits(str)))
	{
		...
		{
			if(evalType == START_TIME_VALIDATION)         /* start time validation */
			{
				sb_send_string(TEXT_ERR_START_IN_PAST_TXT);
			}
			else if(evalType == FINISH_TIME_VALIDATION)    /* finish time validation */
			{
				if(ep < current_epoch)
				{
...
ConfigurationState_t clockConfigurationCheck(void)
{
	...
	if((g_event_finish_epoch < MINIMUM_EPOCH) || (g_event_start_epoch < MINIMUM_EPOCH) || (current_epoch < MINIMUM_EPOCH))
	{
	...
	if(current_epoch > g_event_finish_epoch)  /* The scheduled event is over */
	{
		return(CONFIGURATION_ERROR);
	}

	if(current_epoch > g_event_start_epoch)       /* Event should be running */
	{
		if(!g_event_enabled)
		{
...
void reportConfigErrors(void)
{
	...
	if(current_epoch < MINIMUM_EPOCH) /* Current time is invalid */
	{
		sb_send_string(TEXT_SET_TIME_TXT);
	}

	if(g_event_finish_epoch < current_epoch)      /* Event has already finished */
	{
		if(g_event_start_epoch < current_epoch)   /* Event has already started */
		{
	...
	else if(g_event_start_epoch < current_epoch)  /* Event has already started */
	{
