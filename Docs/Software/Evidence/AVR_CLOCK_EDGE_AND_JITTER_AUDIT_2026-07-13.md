...
The on-air countdown/state slice likewise leaves the same-priority PORTA and TCB0 owners unchanged. Six foreground assignments now use one atomic publisher, and event launch uses its already-computed local result for ID arithmetic before publishing once. The red/green contract, ownership reasoning, generated assembly, and exact build are recorded in [AVR on-air state atomic stores](AVR_ON_AIR_ATOMIC_STORES_2026-07-13.md). Its target gate requires controlled start/stop observation on the dummy load because this state directly drives RF cycling.
...
5. **Make 32-bit transfers explicit.** The event start/finish and wake-time slices pass focused target gates. The separately owned on-air state now protects only foreground publishers and passes source/build gates without changing either ISR; complete its controlled dummy-load start/stop gate next.
...
Long-duration drift and aging-register work remains bookmarked. The RTC edge-loss recovery is source- and target-verified, with broader product regression retained for A8. The event-epoch and wake-time atomic slices pass their focused source, exact-build, programming, and runtime gates without changing either consuming ISR. The separately owned on-air state now passes source/build gates with unchanged ISR bodies and needs a controlled dummy-load start/stop target gate. These changes target rare discrete jitter and whole-second loss, which are better aligned with the observed failure class than expected short-term oscillator drift.
