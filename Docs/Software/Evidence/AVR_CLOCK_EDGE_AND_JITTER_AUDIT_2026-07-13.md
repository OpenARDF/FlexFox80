...
The separately owned wake-time slice protects only the foreground stores. Its TCB0 writer and PORTA reader share normal interrupt priority and therefore cannot preempt each other; adding ISR overhead would not improve that transfer. The red/green contract, ownership reasoning, exact build, byte-verified programming, and focused runtime evidence are recorded in [AVR wake-time atomic stores](AVR_WAKE_TIME_ATOMIC_STORES_2026-07-13.md).

The on-air countdown/state slice likewise leaves the same-priority PORTA and TCB0 owners unchanged. Six foreground assignments now use one atomic publisher, and event launch uses its already-computed local result for ID arithmetic before publishing once. The red/green contract, ownership reasoning, generated assembly, exact build, byte-verified programming, and acknowledged zero-power start/stop target gate are recorded in [AVR on-air state atomic stores](AVR_ON_AIR_ATOMIC_STORES_2026-07-13.md).
...
6. **Add boundary tests.** Red/green host, source/build, exact target installation, and normal-path gates pass for `start-1`, `start`, `start+1`, `finish-1`, `finish`, and `finish+1` using one supplied `now`.
...
Long-duration drift and aging-register work remains bookmarked. RTC edge-loss recovery, event-epoch ownership, wake-time ownership, on-air ownership, bounded RTC synchronization, and exact event-boundary decisions are source- and target-verified, with broader product regression retained for A8. These changes target rare discrete jitter and whole-second loss, which are better aligned with the observed failure class than expected short-term oscillator drift. The remaining ordinary report-latency finding describes the observation path and does not justify a clock-changing firmware patch. The priority-ordered AVR clock-read audit is complete; full scheduled event, RF-cycle, and sleep/wake behavior remains in A8.
