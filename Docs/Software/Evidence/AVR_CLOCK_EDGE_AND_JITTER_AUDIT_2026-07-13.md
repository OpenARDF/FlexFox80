...
The separately owned wake-time slice now protects only the foreground stores. Its TCB0 writer and PORTA reader share normal interrupt priority and therefore cannot preempt each other; adding ISR overhead would not improve that transfer. The red/green contract, ownership reasoning, and exact-build evidence are recorded in [AVR wake-time atomic stores](AVR_WAKE_TIME_ATOMIC_STORES_2026-07-13.md). Connected-target verification remains its next gate.
...
5. **Make 32-bit transfers explicit.** The first slice makes foreground event start/finish stores atomic and applies paired bounds as one transaction; its focused target gate passes. The second slice protects only foreground wake-time stores and passes source/build gates. Complete its target gate, then treat `g_on_the_air` separately because its ownership is more complex.
