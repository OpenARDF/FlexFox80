...
The connected FlexFox is authorized as a writable test unit. Before an AVR write, preserve and verify its unit-specific EEPROM, read fuses for comparison, select an exact schema-compatible artifact, and define the rollback and applicable hardware test plan. Full AVR flash/device backups are optional unless the owner requests one or a separately documented recovery risk requires one. Do not alter fuses or lock bits unless a separate test explicitly requires it.
...
Use Arduino IDE's normal board menu and serial-port workflow for interactive programming. Routine sketch-only updates do not require a full 4 MiB backup: preserve the existing LittleFS by writing only at `0x0`, document bootloader wiring, image offsets, rollback, and post-write verification. Make a full ESP backup only when specifically requested or separately justified.
