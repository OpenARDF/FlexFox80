...
`just esp-build` implements a migration-candidate profile with pinned ESP8266 core 3.1.2 and WebSockets 2.7.2. It uses an isolated ignored Arduino profile, installs exact dependencies when absent, compiles the sketch, creates the 1,024,000-byte LittleFS image, and writes hashes/resource evidence beneath `Software/Huzzah/tmp/esp-build/`. It never uploads firmware.

The exact historical core and deployed ESP binary remain unrecovered. WebSockets 2.1.0 recovered from historical Windows source cannot compile the checked-in firmware; 2.1.1 is incompatible with core 3.1.2; 2.7.2 builds cleanly after replacing one untracked `isRunning()` extension with source-owned lifecycle state. Two clean builds produced the same flashable `.bin`, but the image later failed its installed startup/SSID test and was rolled back. The profile is a repeatable diagnostic candidate, not a qualified release baseline; see [Mac ESP8266 build baseline](Evidence/MAC_ESP8266_BUILD_BASELINE_2026-07-12.md).
...
The first standalone HUZZAH was preserved, programmed with the migration firmware-only image, and independently verified through an FT232R adapter. Manual GPIO0/RESET bootloader entry, `--before no_reset`, and 115200 baud were required; its existing filesystem was retained. Installed startup failed, so the exact full-flash backup was restored and verified. Future candidates must advertise their SSID after standalone RESET before installation. See [Mac ESP8266 programming evidence](Evidence/MAC_ESP8266_PROGRAMMING_2026-07-12.md).
