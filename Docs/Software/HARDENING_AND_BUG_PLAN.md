...
| R13 | A/B | High | AVR RF-cycle concurrency | Foreground on-air state stores are atomic to same-priority ISR owners; event launch publishes one locally calculated value | [On-air state atomic stores](Evidence/AVR_ON_AIR_ATOMIC_STORES_2026-07-13.md); red/green contract and exact zero-warning AVR build pass with unchanged ISR sizes and 52 fewer flash bytes | Controlled dummy-load start/stop gate; A8 RF-cycle regression | Implementing |
