...
| B-FS-07 | B | Critical | ESP LittleFS upload recovery | Power loss between the backup and live-file rename left complete backup/staging files but no live `/test.html`; synchronous startup cleanup withheld HTTP until AVR power-down, repeating on every boot | [ESP interrupted file-upload recovery](Evidence/ESP_INTERRUPTED_FILE_UPLOAD_RECOVERY_2026-07-19.md); exact full-flash/LittleFS retention, ESP 2.23 build/contracts, standalone file hashes, reinstall, and combined ESP/BL/AVR live gate pass | Install ESP 2.23 on every provisioned unit and monitor ordinary fleet file deployment | Corrected; recovered hardware passed |
