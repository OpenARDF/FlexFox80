...
| B-FS-07 | B | Critical | ESP LittleFS upload recovery | Power loss between the backup and live-file rename left complete backup/staging files but no live `/test.html`; synchronous startup cleanup withheld HTTP until AVR power-down, repeating on every boot | [ESP interrupted file-upload recovery](Evidence/ESP_INTERRUPTED_FILE_UPLOAD_RECOVERY_2026-07-19.md); exact 2.23 recovery evidence plus ESP 2.24 no-auto-format, checked-mount, watchdog-escape, sketch-resident recovery service, status, host tests, and build gates | Qualify 2.24 recovery mode and bounded AVR 0.209 maintenance lease on accessible hardware before installing remaining units | Corrected in source; 2.24 hardware gate pending |
...
| B-TEMP-06 | B | Low | AVR ADC temperature reporting | AVR startup can transiently report an impossible temperature; `230.0C` was reproduced twice after reset before six consecutive 22–25 C readings | Fleet-upgrade live probes; fan control already rejects readings outside its sanity range, so the observed defect affected telemetry rather than fan state | Characterize ADC readiness, MUX ownership, conversion timeout state, and first-sample behavior; correct and qualify after the current fleet rollout | Planned wireless AVR service fix |
...
### Deferred wireless AVR service fix: startup temperature

Track the known startup temperature defect as `B-TEMP-06`. The correction should establish a bounded ADC-conversion contract, characterize whether ADC readiness, MUX restoration/ownership, retained timeout state, or the first conversion after reset produces the invalid sample, and reject rather than publish a conversion outside the existing fan-control sanity range. The fan and telemetry paths should reuse the same validated reading rule so they cannot drift.

Add host/source characterization where practical, then qualify repeated cold starts, software resets, ESP-requested temperature reports, battery/temperature ADC interleaving, and the 30/35 C fan hysteresis on a representative unit. Deliver the correction as a normal qualified wireless AVR application update after the current provisioning and soak cycle; it does not require another chassis-open fleet interval or a bootloader revision. Until then, the fleet-upgrade wrapper records and retries out-of-range startup telemetry instead of accepting it as PASS evidence.
