...
| R10 | A/B | High | AVR RTC/ISR timing | Level-1 counted-edge candidate recovers coalesced RTC ticks without changing RF keying or standby clocks | [RTC edge recovery](Evidence/AVR_RTC_EDGE_RECOVERY_2026-07-13.md) and [target verification](Evidence/AVR_RTC_EDGE_TARGET_VERIFICATION_2026-07-13.md); exact programming/readback pass, runtime delay/fault injection pending | A5/A6 | Implementing |
...
For active `B-TIME-01`, long-duration drift and aging-register work remain deliberately postponed. The [AVR clock-edge and jitter audit](Evidence/AVR_CLOCK_EDGE_AND_JITTER_AUDIT_2026-07-13.md) identifies retrying Si5351 I2C inside normal-priority interrupts as a credible whole-second tick-loss path. The first conservative candidate is documented in [AVR RTC edge recovery](Evidence/AVR_RTC_EDGE_RECOVERY_2026-07-13.md): it counts RTC edges with the already-running Level-1 timeout timer, replays elapsed seconds, preserves the newest clone one-shot epoch, adds 168 bytes of flash and 3 bytes of BSS, and passes host contracts plus an exact zero-warning AVR build. Exact target programming/readback, restored EEPROM, and unchanged fuses pass in [AVR RTC edge target verification](Evidence/AVR_RTC_EDGE_TARGET_VERIFICATION_2026-07-13.md). The next gate is read-only WiFi/AVR runtime verification after restoring the Moto USB/tether link, followed by a controlled delay spanning multiple RTC edges. Only after that gate should work proceed to atomic transfer of the audited multi-byte schedule state.
