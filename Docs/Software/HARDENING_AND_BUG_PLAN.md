...
| B-LINK-04 | B | High | ESP/AVR Linkbus transaction | Static review: event NAK can be reported as success; failed transfer leaves stale frames queued; AVR ACK can mask apply failure; `PRM` can persist a partial event; AVR output drops are silent | [ESP-to-AVR Linkbus reliability review](Evidence/ESP_AVR_LINK_RELIABILITY_REVIEW_2026-07-17.md); red-green outcome tests, source contract, and exact ESP 2.6 build pass | Pilot ESP 2.6 on sprint master and fox 1; verify restart persistence and failure recovery; defer AVR semantic/atomic protocol changes to a justified service release | Implemented; pilot pending |
