...
| B-LINK-04 | B | High | ESP/AVR Linkbus transaction | Static review: event NAK can be reported as success; failed transfer leaves stale frames queued; AVR ACK can mask apply failure; `PRM` can persist a partial event; AVR output drops are silent | [ESP-to-AVR Linkbus reliability review](Evidence/ESP_AVR_LINK_RELIABILITY_REVIEW_2026-07-17.md); current host suite passes but has no transaction/ACK/queue fault matrix | Implement and host-test ESP-only isolated fail-fast transaction; pilot on sprint master and fox 1; defer AVR semantic/atomic protocol changes to a justified service release | Characterized; ESP fix planned |
