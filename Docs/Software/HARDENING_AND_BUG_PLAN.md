...
Path A has resumed after the priority-ordered AVR timing audit. The confirmed ESP role-index correction now passes source/build gates with a 32-byte flash-only increase. Complete its distinct-role cached-value/AVR-message target fixture and restore normal configuration next; do not combine it with broader numeric validation or event-file integrity work.
