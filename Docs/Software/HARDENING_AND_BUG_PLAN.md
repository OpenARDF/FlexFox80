...
| B-TIME-01 | B | High | ESP/Linkbus/AVR RTC | Rare field outlier; reset-dependent one-second system-time quantization reproduced | AVR/ESP clone controls pass; exact readback passes; master and target resets independently shift reported phase by about one second; edge-aligned boot candidate builds | Program dummy-loaded target, run varied-phase reset series, then qualify corrected master/target clone and retain drift evidence | Implementing |
