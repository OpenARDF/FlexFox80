...
For active `B-TIME-01`, preserve unit identity, elapsed time since setting, median offset, observation spread, and RTC aging value before changing calibration. The updated master-target work is documented in [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md): clone cycles crossed the exact RTC-readback gate and placed the target approximately 0.54–0.57 seconds behind the master, but separate master and target resets each produced a near-one-second system-time phase shift. The immediate boot RTC read followed by unconditional ISR ticking is the leading reproduced mechanism. A failing source contract preceded the minimal edge-aligned boot correction. The programmed target and master pass their post-program-plus-five-reset gates within 81 ms and 39.5 ms boot-median ranges respectively. The first two corrected clones transferred all nine files, cleaned up normally, and produced paired 479–548 ms target lags; an intervening master reset changed its median only -7.5 ms. Drift evidence and a broader convenient clone sample are next.
