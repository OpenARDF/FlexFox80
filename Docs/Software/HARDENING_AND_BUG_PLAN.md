...
| B-TIME-01 | B | High | ESP/Linkbus/AVR RTC | Rare field outlier; reset-dependent one-second system-time quantization reproduced | AVR/ESP clone controls and exact readback pass; edge-aligned target passes programming and five-reset gate with 81 ms median range and no whole-second shift | Preserve/program master, run its reset gate, then qualify corrected master/target clone and retain drift evidence | Implementing |
