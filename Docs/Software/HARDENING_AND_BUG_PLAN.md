...
**Plan status:** Step A1 is complete on `Development_AVR128DA48`; the exact AVR build is reproducible on Mac and Windows, the pinned ESP8266 core-2.7.4/WebSockets-2.3.6 build is hardware-compatible, and characterization-first TDD plus narrow firmware hardening slices are active.
...
- [x] Hardware characterization rejected core 3.1.2/WebSockets 2.7.2 and core 3.0.2/WebSockets 2.3.6 after pre-clone images repeatedly reset standalone. Core 2.7.4/WebSockets 2.3.6 passes standalone startup with pre-clone and clone-sync source, installed WiFi/AVR probing, and single-unit clone controls.
- [x] The historical WebSockets boundary is explicit: 2.1.0 lacks APIs used by the checked-in source, later current releases are incompatible with older cores, and an untracked `isRunning()` extension was replaced with source-owned lifecycle state.
...
- [x] AVR deployed/source identity is established; the preserved mature ESP image remains an exact rollback separate from the hardware-compatible development build.
...
- Direct host tests protect canonical byte encodings and prove known `KEY`, `RST`, `GO`, and `TEM` aliases are distinct.
...
- The dummy-loaded test unit now runs the exact corrected flash; independent flash, complete EEPROM, and fuse readbacks match the intended and preserved bytes.
- The connected target rejects the legacy read-only `$RXW?` temperature alias without a `TEMP` response, and a following battery query proves recovery.
...
**Confirmed ESP role-assignment defect:**
...
- A one-digit role such as `"1:0"` therefore produces an empty substring and resolves the cached power/frequency using role zero; longer prefixes lose their final digit.
- Descriptive-name parsing elsewhere correctly uses `substring(0, c)`, making the mismatch locally unambiguous.
- A completed call-site trace found that the stale cache does not drive current AVR programming: that path parses the intact stored assignment and performs direct per-role lookups. This is an internal-state correction and latent-risk removal, not a demonstrated cause of released AVR misconfiguration.
- The hardware-compatible ESP8266 core and LittleFS profile is now qualified. The [role-assignment correction](Evidence/ESP_ROLE_ASSIGNMENT_2026-07-13.md) passes its red/green host test, source contract, immediate-baseline comparison, two exact candidate sketch builds, independent flash verification, standalone startup, guarded installed role-persistence/restoration gate, clone controls, and final HTTP/WebSocket/AVR probe. R8 is complete.
...
| R4 | A | Medium | ESP event files | Legacy files remain compatible; clone `/Temp` requires the transferred checksum | [ESP clone event-file integrity](Evidence/ESP_EVENT_FILE_INTEGRITY_2026-07-13.md); red/green host tests, exact builds, installed smoke test, normal clone, controlled mismatched-checksum rejection, prior-file retention, and two-unit restoration pass | — | Complete |
...
| R8 | A/B | Medium | ESP role assignment | Complete role prefixes select cached power/frequency for role 0, role 1, and the multi-digit parser boundary | [Role-assignment correction](Evidence/ESP_ROLE_ASSIGNMENT_2026-07-13.md); red/green bounds test, source contract, exact builds/flash, standalone and installed gates pass; original role restored; current AVR path does not consume the stale cache | — | Complete |
...
| R10 | A/B | Critical | AVR RTC/ISR timing | AVR `0.201` replayed alternating sleeping wakes as 255 seconds; the first `0.202` target gate also exposed an expired pre-start sleep mode when WiFi remained connected across event start | [RTC edge recovery](Evidence/AVR_RTC_EDGE_RECOVERY_2026-07-13.md), [target verification](Evidence/AVR_RTC_EDGE_TARGET_VERIFICATION_2026-07-13.md), and [sleep/wake regression](Evidence/AVR_SLEEP_WAKE_RTC_COUNTER_REGRESSION_2026-07-17.md); AVR `0.203` source/host/build gates and dummy-loaded scheduled start/cycle/finish/sleep pilot pass | Complete synchronized fleet rehearsal and extended soak | Corrected; fleet gate pending |
| R11 | A/B | High | AVR schedule concurrency | Foreground event start/finish stores are atomic to ISR readers; paired bounds become visible as one transaction | [Event-epoch atomic stores](Evidence/AVR_EVENT_EPOCH_ATOMIC_STORES_2026-07-13.md); red/green contract, exact zero-warning AVR build, byte-verified programming, and focused runtime gate pass | A8 broader regression | Complete |
| R12 | A/B | High | AVR sleep/wake concurrency | Foreground wake-time stores are atomic to the RTC ISR; same-priority TCB0-to-PORTA ownership remains unchanged | [Wake-time atomic stores](Evidence/AVR_WAKE_TIME_ATOMIC_STORES_2026-07-13.md); red/green contract, exact zero-warning AVR build, byte-verified programming, and focused runtime gate pass without resource growth | A8 sleep/wake regression | Complete |
| R13 | A/B | High | AVR RF-cycle concurrency | Foreground on-air state stores are atomic to same-priority ISR owners; event launch publishes one locally calculated value | [On-air state atomic stores](Evidence/AVR_ON_AIR_ATOMIC_STORES_2026-07-13.md); red/green contract, exact zero-warning AVR build, byte-verified programming, and acknowledged zero-power start/stop gate pass | A8 RF-cycle regression | Complete |
| R14 | A/B | High | AVR RTC synchronization | RTC-edge waits are bounded and RTC reads cannot commit across a counted edge | [Bounded RTC synchronization guard](Evidence/AVR_RTC_SYNC_GUARD_2026-07-13.md); red/green host tests, exact build, isolated missing-edge/intervening-edge target injection, exact flash restoration, byte-preserved EEPROM/fuses, and normal clock write/readback pass | A8 broader regression | Complete |
...
| B-RTC-05 | B | Critical | AVR sleep/wake scheduler | Ten synchronized Sprint units missed a future start; reset temporarily restored operation before premature finish; first target retest also rephased after delayed WiFi shutdown | [Sleep/wake RTC counter regression](Evidence/AVR_SLEEP_WAKE_RTC_COUNTER_REGRESSION_2026-07-17.md); exact `1,255` sequence reproduced, full-day host regression and connected-start contract pass, deterministic zero-warning AVR `0.203` builds pass, and the pilot proves pre-event/inter-cycle/post-event sleep plus exact future wakes | Complete synchronized fleet rehearsal and extended soak | Corrected; pilot passed; fleet gate pending |
...
| B-CLONE-02 | B | Medium | ESP clone file transfer | Operator estimates retry needed in 5–10% of clones; fleet report records 4/23 first-attempt misses (17.4%), all 4 succeeding on retry | [ESP-to-AVR Linkbus reliability review](Evidence/ESP_AVR_LINK_RELIABILITY_REVIEW_2026-07-17.md) records the field clone tail and the misleading solid-LED recovery state; one six-of-nine-file bench stall followed by a successful retry; first two corrected clones pass | Add phase-specific retained clone diagnostics and characterize bounded retry/recovery independently of B-TIME-01 | Field failure tail confirmed; root phase open |
| B-HTTP-03 | B | Medium | ESP startup/HTTP/Linkbus | Wi-Fi association can precede HTTP availability by almost three minutes | [ESP HTTP startup delay](Evidence/ESP_HTTP_STARTUP_DELAY_2026-07-16.md); live first response at 178.332 seconds is confirmed, but the prior exact 153-second pre-HTTP event-queue attribution is corrected | Add startup phase timestamps; design an early readiness service that preserves automatic cloning | Partly characterized; trigger open |
| B-FS-07 | B | Critical | ESP LittleFS upload recovery | Power loss between the backup and live-file rename left complete backup/staging files but no live `/test.html`; synchronous startup cleanup withheld HTTP until AVR power-down, repeating on every boot | [ESP interrupted file-upload recovery](Evidence/ESP_INTERRUPTED_FILE_UPLOAD_RECOVERY_2026-07-19.md); exact 2.23 recovery plus ESP 2.24/AVR 0.209 build, recovery-only install/restore, wireless AVR update, 135.817-second maintenance upload, cleanup, and return-to-normal shutdown gates pass | Install 2.24/0.209 on remaining opened units; retain 2.22 controls and monitor ordinary event/RF/power-down soak | Corrected; pilot passed |
| B-LINK-04 | B | High | ESP/AVR Linkbus transaction | Static review: event NAK can be reported as success; failed transfer leaves stale frames queued; AVR ACK can mask apply failure; `PRM` can persist a partial event; AVR output drops are silent | [ESP-to-AVR Linkbus reliability review](Evidence/ESP_AVR_LINK_RELIABILITY_REVIEW_2026-07-17.md); red-green outcome tests, source contract, exact ESP 2.6 build, master/target install, restart persistence, and three consecutive live clone/sync operations pass | Proceed with identity-first ESP-only fleet rollout and per-unit verification; monitor retained diagnostics; defer AVR semantic/atomic protocol changes to a justified service release | Implemented; pilot passed |
| B-TEMP-06 | B | Low | AVR ADC temperature reporting | AVR startup can transiently report an impossible temperature; `230.0C` was reproduced twice after reset before six consecutive 22–25 C readings | [AVR startup temperature validity](Evidence/AVR_TEMPERATURE_VALIDITY_2026-07-19.md); root cause is competing foreground/TCB0 ADC ownership, acceptance of a prior channel's ready result, partial state restoration, and a non-resetting static timeout; ESP 2.25 / AVR 0.210 passed host, zero-warning build, wireless install, exact readback, and three-reset telemetry gates | Complete cold-power, unavailable/fan, and normal-operation pilot gates | Corrected; reset pilot passed |
| B-VOLT-08 | B | Low | AVR ADC / ESP voltage telemetry | Two units that had remained in standby for days initially reported approximately 1.2 V after antenna-removal wake, then reported the correct 12.1–12.2 V | Source trace shows that standby wake retains the cached ADC result and scanner state, `$BAT?` returns that cache without requiring a fresh post-wake sample, and the ESP's next automatic battery request can be nearly 17 seconds later | Add a focused post-standby freshness regression and repeated antenna-wake target qualification before changing the ADC or sleep/wake path | Planned — near-term |
...
`B-HTTP-03` and `B-LINK-04` are post-release hardening items rather than authorization for an immediate fleet reflash. The first correction should be ESP-only: isolate and bound event programming, preserve automatic cloning priority, prevent stale queue work after failure, add phase diagnostics, and gate configuration until initialization is safe. It must pass the pilot-master/target fault matrix before wider deployment. AVR semantic acknowledgements and atomic event commit should wait for a separately justified service release unless pilot evidence proves an ESP-only correction inadequate.

### Wireless AVR service fix: startup temperature

`B-TEMP-06` was authorized during the 2.0.0 provisioning work. The source confirms two independent failure mechanisms: foreground temperature reads competed with the TCB0 battery/rail ADC owner and could consume a ready result from the previous channel, while the helper's `static` timeout was never reset and could permanently fall through to its numeric sentinel. ESP 2.25 / AVR 0.210 replace that path with one scheduled ADC owner, a 10 ms conversion deadline, an early startup sample, an explicit availability flag, inclusive `-20C` through `120C` validation, and fan-on fail-safe behavior while unavailable.

AVR unavailable telemetry is now `!TEM,NA`; ESP and all three user pages display it as unavailable and independently reject malformed or out-of-range values. Host/source, exact-build, wireless-install, exact-readback, and three software-reset gates pass on `Tx_C22DD117`. Repeated whole-unit cold starts, longer ADC interleaving, unavailable/fan behavior, the 30/35 C fan hysteresis, and normal-event operation remain the representative hardware gate. The correction is an ordinary wireless AVR application update and does not change BL0.3.

### Near-term telemetry fix: post-standby external-voltage freshness

`B-VOLT-08` is scheduled for the near-term hardening cycle, but this roadmap entry does not authorize an immediate firmware or fleet change. The observed low value is an AVR-provided cached ADC result rather than a browser decimal-formatting error. Antenna-removal wake reinitializes the active system without explicitly invalidating the retained external-voltage result or ADC scanner state; the battery request replies immediately from that cache. After the first reply, the ESP may wait nearly 17 seconds before requesting battery voltage again, allowing a transient or stale value to remain visible after the AVR has resumed normal sampling.

The fix should preserve the existing RF, event, sleep, and temperature behavior while establishing one explicit contract: no external-voltage value is reported as current until it comes from a valid conversion taken after the latest wake. Start with a failing host/source regression for that freshness boundary, then make the smallest owning-subsystem change needed to:

1. invalidate the external-voltage cache and reinitialize the ADC scanner consistently after standby wake;
2. obtain a fresh sample after the powered rails and ADC reference have settled;
3. return an unavailable indication or defer the reply rather than publishing a pre-wake or unvalidated value;
4. retry promptly during startup, then resume the ordinary low-rate telemetry interval; and
5. add a discarded settling conversion or longer acquisition time only if target evidence shows that channel switching still corrupts the first sample.
...
1. **AVR bootloader liveness and integrity:** characterize every currently indefinite NVM-busy, serial-transmit, and related wait; evaluate a bootloader-scoped watchdog with classified reset diagnostics; and evaluate additional redundant boot/application self-checks. Any implementation must preserve the persistent recovery marker, reset-page-last commit, ordered/idempotent page protocol, protected boot range, and safe ESP power/reset handoff. It requires host fault injection plus uninterrupted, ESP-reset, AVR-reset, brownout, true-power-loss, and exact UPDI readback qualification on representative units before fleet installation.
2. **ESP installation rollback and recovery:** evaluate a flash-layout and boot-flow design that retains a known-good wireless recovery image or provides dual-image rollback when a staged sketch cannot boot or power is lost during the eboot copy. Re-enabling a post-copy comparison alone is not sufficient because it detects corruption without restoring the overwritten image. The design must preserve LittleFS and unit-specific files, keep a recovery updater reachable without the main application, reject unqualified images, and pass repeated upload, reboot, copy-interruption, rollback, and normal-operation testing before replacing the current eboot/application update path.

These are resilience opportunities, not known release-blocking defects. Do not trade the qualified BL0.3/current ESP behavior for either design without a separately reviewed recovery architecture and a complete boot-chain requalification plan.
...
The immediate release blocker is `B-RTC-05`. AVR `0.201` is withdrawn from scheduled-operation use. The AVR `0.203` dummy-loaded pilot now passes a real future scheduled start, repeated Sprint timing beyond the original failure point, Classic pre-event and inter-transmission sleep, exact finish handling, post-event sleep with a future event scheduled, and cold-power recovery. Do not reflash the Championship fleet until a complete synchronized rehearsal and extended soak pass on the ten units.

For active `B-TIME-01`, long-duration drift and aging-register work remain deliberately postponed. The [AVR clock-edge and jitter audit](Evidence/AVR_CLOCK_EDGE_AND_JITTER_AUDIT_2026-07-13.md) identified retrying Si5351 I2C inside normal-priority interrupts as a credible whole-second tick-loss path. The conservative recovery in [AVR RTC edge recovery](Evidence/AVR_RTC_EDGE_RECOVERY_2026-07-13.md) counts RTC edges with the already-running Level-1 timeout timer, replays elapsed seconds, preserves the newest clone one-shot epoch, adds 168 bytes of flash and 3 bytes of BSS, and passes host contracts plus an exact zero-warning AVR build. [Target verification](Evidence/AVR_RTC_EDGE_TARGET_VERIFICATION_2026-07-13.md) held the normal-priority ISR for approximately 3.004 seconds; the AVR clock caught up four seconds across the suppressed report and its post-test offset median shifted only -79 ms. Production flash, EEPROM, and fuses were then independently restored byte-for-byte. The event-epoch, [wake-time ownership](Evidence/AVR_WAKE_TIME_ATOMIC_STORES_2026-07-13.md), and [on-air ownership](Evidence/AVR_ON_AIR_ATOMIC_STORES_2026-07-13.md) slices pass their focused source, exact-build, byte-verified programming, and runtime gates without changing either consuming ISR. The [bounded RTC synchronization guard](Evidence/AVR_RTC_SYNC_GUARD_2026-07-13.md) replaces both remaining infinite edge waits and rejects failed or edge-crossed RTC reads. Isolated missing-edge and intervening-edge injection both returned error 252 without stranding foreground processing; exact production flash restoration, byte-preserved EEPROM/fuses, advancing telemetry, and the guarded clock write/readback cycle all pass. R14 is complete, with broad RF-cycle and sleep/wake regression retained in A8. The [event-boundary state](Evidence/AVR_EVENT_BOUNDARY_STATE_2026-07-13.md) now passes deterministic `start-1` through `finish+1` tests using one supplied `now`, two exact builds, byte-verified target installation, preserved EEPROM/fuses, and normal advancing telemetry. R15 and the priority-ordered AVR clock-read audit are complete; the remaining ordinary report-latency finding is observational, while full scheduled event, RF-cycle, and sleep/wake behavior remains in A8.

The deterministic AVR slice is documented in [AVR clone synchronization controls](Evidence/AVR_CLONE_SYNC_CONTROLS_2026-07-12.md). [ESP clone synchronization controls](Evidence/ESP_CLONE_SYNC_CONTROLS_2026-07-12.md) now quiet both AVRs, wait for the master's next-edge report, queue the target write, require an exact clone-specific RTC readback, and resume reports on cleanup. Characterization isolated ESP startup compatibility to core 2.7.4/WebSockets 2.3.6; the clone-sync image passes standalone startup, installed WiFi/AVR probing, and live quiet/one-shot-edge/resume. Single-unit queued-write characterization found correct logical epochs and only a 55 ms circular mean phase shift, while multi-second report delays occurred before writes and disappeared on following edges. The updated master has independently verified ESP and AVR images; its clean historical 268-byte EEPROM was migrated to the corrected 274-byte schema before restoration. Installed master HTTP, WebSocket, AVR telemetry, preserved-role, and clone-control gates pass. Clone cycles cross the target RTC-readback gate and reproduce the expected approximately half-second target lag, while reset isolation proves a separate approximately one-second AVR boot quantization on both units. The edge-aligned boot candidate passes its source contract, exact warning-free Mac build, byte-identical flash/EEPROM/fuse verification on both units, and five-reset gates with no whole-second phase change. The first two corrected clones cross the exact readback gate, transfer all nine files, complete normal cleanup, and produce paired 479–548 ms target lags; the intervening master reset changes median phase only -7.5 ms. Broader clone-tail evidence, medium-severity `B-CLONE-02`, physical timing, and drift remain open; do not treat WebSocket receipt time alone as RTC phase. See [Master clone-synchronization firmware upgrade](Evidence/MASTER_CLONE_SYNC_UPGRADE_2026-07-13.md) and [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md).

The ESP role-index correction and R4 event-file checksum correction now pass their complete source, build, installed, target, and restoration gates. The release behavior freeze is active. Continue with reproducible release-candidate checks, artifact identity, release notes, rollback references, and branch-readiness review; do not begin another firmware behavior slice without a newly reproduced release-blocking regression.

When hardening resumes after the release interval, the next AVR path is numeric input rejection. Begin by defining invalid, negative, overflow, and partial-number behavior for time, interval, frequency, power, and aging fields; mature state-mutating behavior must not be guessed or changed as one broad cleanup.
