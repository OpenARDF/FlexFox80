...
| B-TIME-01 | B | High | ESP/Linkbus/AVR RTC | Rare field outlier; repeated phase/drift series pending | AVR/ESP controls built; two full clone/readback cycles pass; first same-path comparison finds target about 0.54–0.56 s behind master | Repeat clone phase measurements; preserve 24-hour/multi-day drift and physical timing evidence | Implementing |
...
For active `B-TIME-01`, repeat the complete clone-plus-immediate-observation sequence and preserve unit identity, elapsed time since setting, median offset, observation spread, and RTC aging value before changing calibration. The first updated master-target trial is documented in [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md): two complete clone cycles crossed the exact RTC-readback gate, and the final same-path observation placed the target approximately 0.54–0.56 seconds behind the master rather than multiple seconds away. Determine the repeated phase distribution and whether an outlier drifts gradually or jumps by integral seconds.

The deterministic AVR slice is documented in [AVR clone synchronization controls](Evidence/AVR_CLONE_SYNC_CONTROLS_2026-07-12.md). [ESP clone synchronization controls](Evidence/ESP_CLONE_SYNC_CONTROLS_2026-07-12.md) now quiet both AVRs, wait for the master's next-edge report, queue the target write, require an exact clone-specific RTC readback, and resume reports on cleanup. Characterization isolated ESP startup compatibility to core 2.7.4/WebSockets 2.3.6; the clone-sync image passes standalone startup, installed WiFi/AVR probing, and live quiet/one-shot-edge/resume. Single-unit queued-write characterization found correct logical epochs and only a 55 ms circular mean phase shift, while multi-second report delays occurred before writes and disappeared on following edges. The updated master has independently verified ESP and AVR images; its clean historical 268-byte EEPROM was migrated to the corrected 274-byte schema before restoration. Installed master HTTP, WebSocket, AVR telemetry, preserved-role, and clone-control gates pass. Two full clone cycles now cross the target RTC-readback gate, and the first sequential same-path measurement finds no immediate multi-second phase error. Repeated clone phase spread, physical edge/transmission timing, cleanup failures, and drift remain open; do not treat WebSocket receipt time alone as RTC phase. See [Master clone-synchronization firmware upgrade](Evidence/MASTER_CLONE_SYNC_UPGRADE_2026-07-13.md) and [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md).
