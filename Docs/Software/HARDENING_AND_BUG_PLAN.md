...
| R10 | A/B | Critical | AVR RTC/ISR timing | Awake counted-edge recovery passes, but AVR `0.201` replays alternating sleeping wakes as 255 seconds because the sampler is stopped in standby | [RTC edge recovery](Evidence/AVR_RTC_EDGE_RECOVERY_2026-07-13.md), [target verification](Evidence/AVR_RTC_EDGE_TARGET_VERIFICATION_2026-07-13.md), and [sleep/wake regression](Evidence/AVR_SLEEP_WAKE_RTC_COUNTER_REGRESSION_2026-07-17.md); AVR `0.202` source/host/build gates pass | Complete dummy-loaded scheduled start/Sprint/finish pilot | Reopened; correction pending target gate |
...
| B-RTC-05 | B | Critical | AVR sleep/wake scheduler | Ten synchronized Sprint units missed a future start; reset temporarily restored operation before premature finish | [Sleep/wake RTC counter regression](Evidence/AVR_SLEEP_WAKE_RTC_COUNTER_REGRESSION_2026-07-17.md); exact `1,255` counter sequence reproduced, full-day sleep-wake host regression passes, neighboring timing/RF audit complete, two deterministic zero-warning AVR `0.202` builds pass | Dummy-loaded pilot and complete synchronized fleet rehearsal | Corrected in source; hardware gate pending |
...
The immediate release blocker is `B-RTC-05`. AVR `0.201` is withdrawn from scheduled-operation use. Qualify AVR `0.202` first on one dummy-loaded pilot with a real future scheduled start, repeated Sprint sleep/wake cycles, an extended soak, exact scheduled finish, and cold-power recovery. Do not reflash the Championship fleet until that pilot and a complete synchronized rehearsal pass.
