...
| B-TIME-01 | B | High | ESP/Linkbus/AVR RTC | Rare field outlier; reset-dependent one-second system-time quantization reproduced | Corrected target/master pass reset gates; first two corrected clones transfer all files with 479–548 ms target lags | Retain drift and broader convenient clone evidence | Implementing |
...
For active `B-TIME-01`, preserve unit identity, elapsed time since setting, median offset, observation spread, and RTC aging value before changing calibration. The updated master-target work is documented in [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md): clone cycles crossed the exact RTC-readback gate and placed the target approximately 0.54–0.57 seconds behind the master, but separate master and target resets each produced a near-one-second system-time phase shift. The immediate boot RTC read followed by unconditional ISR ticking is the leading reproduced mechanism. A failing source contract preceded the minimal edge-aligned boot correction. The programmed target and master pass their post-program-plus-five-reset gates within 81 ms and 39.5 ms boot-median ranges respectively. The first two corrected clones transferred all nine files, cleaned up normally, and produced paired 479–548 ms target lags; an intervening master reset changed its median only -7.5 ms. The drift baseline is anchored to 2026-07-13 20:22–20:24 UTC. The AVR can query its DS3231 aging register, but the installed ESP does not expose that reply over WiFi; defer instrumentation/reflash until drift evidence justifies it.
