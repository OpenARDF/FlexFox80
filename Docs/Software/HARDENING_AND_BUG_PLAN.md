...
| R15 | A/B | Medium | AVR event boundaries | Foreground schedule decisions use one `now` and match the RTC ISR's `start <= now < finish` interval | [Event-boundary state](Evidence/AVR_EVENT_BOUNDARY_STATE_2026-07-13.md); red/green boundary tests, source contract, generated code, and two exact zero-warning builds pass with 144 fewer flash bytes | Exact target installation and normal-path probe | Implementing |
...
When Path A resumes, the next ordered defect is the confirmed ESP role-index extraction error. The hardware-compatible pinned toolchain removes its former build/startup blocker, but implementation remains behind completion or an explicit re-bookmarking of the active clone-time investigation.
