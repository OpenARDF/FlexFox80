...
| R10 | A/B | High | AVR RTC/ISR timing | Retrying Si5351 I2C is reachable from normal-priority ISRs and can coalesce RTC port edges | [Clock-edge and jitter audit](Evidence/AVR_CLOCK_EDGE_AND_JITTER_AUDIT_2026-07-13.md); hardware fault injection pending | A5/A6 | Planned |
...
For active `B-TIME-01`, preserve unit identity, elapsed time since setting, median offset, observation spread, and RTC aging value before changing calibration. The updated master-target work is documented in [Two-unit clone synchronization qualification](Evidence/TWO_UNIT_CLONE_SYNC_2026-07-13.md): clone cycles crossed the exact RTC-readback gate and placed the target approximately 0.54–0.57 seconds behind the master, but separate master and target resets each produced a near-one-second system-time phase shift. The immediate boot RTC read followed by unconditional ISR ticking is the leading reproduced mechanism. A failing source contract preceded the minimal edge-aligned boot correction. The programmed target and master pass their post-program-plus-five-reset gates within 81 ms and 39.5 ms boot-median ranges respectively. The first two corrected clones transferred all nine files, cleaned up normally, and produced paired 479–548 ms target lags; an intervening master reset changed its median only -7.5 ms. The drift baseline is anchored to 2026-07-13 20:22–20:24 UTC, but long-duration drift and aging-register work are deliberately postponed. The [AVR clock-edge and jitter audit](Evidence/AVR_CLOCK_EDGE_AND_JITTER_AUDIT_2026-07-13.md) is the current timing-hardening bookmark: it identifies retrying Si5351 I2C inside normal-priority interrupts as a credible whole-second tick-loss path and identifies non-atomic 32-bit schedule state for directed characterization. The AVR can query its DS3231 aging register, but the installed ESP does not expose that reply over WiFi; do not add instrumentation or change calibration until drift work is resumed explicitly.
